...
function Payment(){
    ...
    // <div class='purchase_btn_text'>by Credit Card </div>
    this.payByPaypal  =$("<div id='paypal-button'>").addClass("payby_btn").append("<form action='https://www.paypal.com/cgi-bin/webscr' method='post' target='_top'><input type='hidden' name='cmd' value='_s-xclick'><input type='hidden' name='hosted_button_id' value='DKLDZ4TC665AW'><img class='icon' src='ico/pay_paypal.png'><div class='purchase_btn_text'>by Credit Card </div></form>")
...
function paymentByPaypal(){
  ...
  paypal.Button.render({
            ...
            onAuthorize: function(data, actions) {
                this.paypal_data = data;
                purchaseController.addPurchase(3);
                // Execute the payment here, when the buyer approves the transaction
                // data에 transactionid, 결제 승인 정보 담겨있음
           }
...
function paymentByiOS(){
  inAppPurchase
    ...
    .then(function () {
      ...
      this.purchaseProvider = 2;
      purchaseController.addPurchase(3);

      console.log('아이폰인앱 결제 성공');
 ...
 */
 ...
 function paymentByCode(){
     ...
     $(".payment_btn").click(function(){
             ...
             showPurchaseSpinner();
             var data = {
               key : purchase_input.text,
               userid : userInfo.userid
             };
             AjaxCall.call(apiUrl + "/purchase/checkcode",
         			data,
         			"POST",
         			function (result) {
                 if(result.status == 'possible'){
                     this.key = purchase_input.text;
                     this.purchaseProvider = 5;
...
function checkPaymentPlatform(){
  ...
  var unknown = '-';
      ...
      //cordova build 시 android project : android return 하여 빌드
      //                ios project : ios return 하여 빌드 -
...
function checkPaymentReward(){
    ...
    // if(val(userInfo.earnedRewardMoney) - var(userInfo.usedRewardMoney) > 2){
        return true;
    // }
    // else{
    //   return false;
...
PurchaseController.prototype = {
    ...
    var purchaseId = '';
    if(this.purchaseProvider == 1){
      purchaseId = this.paypal_data.transactionId;
    }
    else if(this.purchaseProvider == 2){
      purchaseId = this.purchaseId;
    }
    else if(this.purchaseProvider == 3){
      purchaseId = this.purchaseId;
    }
    else if(this.purchaseProvider == 4){

    }
    else if(this.purchaseProvider == 5){
      purchaseId = this.purchaseId;
    }
		var data = {
			userId: userID,
      ...
      purchaseProvider:this.purchaseProvider,
